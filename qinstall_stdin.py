...
loader+="-S %s"%shell
if file_is_sh:
	os.system("qsub -cwd %s %s %s"%(joinargv(sys.argv[1:1+i]),loader,joinargv(sys.argv[i:])))
else:
	#file is not sh, so we need to wrap it with pseudo shell script.
	proc = subprocess.Popen(['qsub','-cwd',joinargv(sys.argv[1:1+i]),loader],stdin=subprocess.PIPE)
	proc.communicate(joinargv(sys.argv[i:])+"\n")
	proc.stdin.close()
